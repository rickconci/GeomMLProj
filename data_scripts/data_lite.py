...
class MIMICContrastivePairsDatasetLite(Dataset):
    def __init__(self, 
        ...
        if splits_loader is not None:
            # For baseline retrieval
            self.event_processor = splits_loader
            
            # For discharge note processing
            if task_mode == 'CONTRASTIVE' and hasattr(splits_loader, 'discharge_df'):
                from .data_lite import MIMICDischargeNotesProcessor
                self.disch_notes_processor = MIMICDischargeNotesProcessor(
                    splits_loader.get_hadm_ids(),
                    cache_dir,
                    discharge_df=splits_loader.discharge_df
                )
            else:
                # Create an empty processor or dummy object for NEXT_24h mode
                self.disch_notes_processor = None
        ...
        if splits_loader is not None and splits_loader.train_hadm_ids is not None:
            ...
            raise ValueError("splits_loader is required for MIMICContrastivePairsDatasetLite")
    ...
    def __getitem__(self, idx):
        ...
        if self.task_mode == 'CONTRASTIVE':
            ...
            # Optionally, load discharge chunks.
            discharge_start = time.time()
            discharge_chunks = None
            if self.disch_notes_processor is not None:
                try:
                    discharge_chunks = self.disch_notes_processor.get_discharge_chunks(hadm_id, bypass_lmdb=False)
                except Exception as e:
                    print(f"Error loading discharge chunks for hadm_id {hadm_id}: {e}")
                    discharge_chunks = []  # Empty list as fallback
            debug_print(f"  - Loading discharge chunks took {time.time() - discharge_start:.2f}s")
            ...
            return {
                ...
                'time_hours_tensor': abs_time_tensor,
                'length': full_length,
                'baseline_tensor': full_baseline_tensor,
                'treatments_df': None,
                'discharge_chunks': discharge_chunks,
            }
        elif self.task_mode == 'NEXT_24h':
            # Similar timing steps can be added for NEXT_24h:
            ...
            if os.path.exists(tensor_cache_path):
                try:
                    ...
                    return {
                        'hadm_id': hadm_id,
                        'physio_tensor': full_physio_tensor,
                        'mask_tensor': full_mask_tensor,
                        'time_hours_tensor': abs_time_tensor,
                        'length': full_length,
                        'baseline_tensor': full_baseline_tensor,
                        'treatments_df': None,
            ...
            if os.path.exists(full_cache_path):
                ...
                # Remove this sample from future iterations
                self.samples = [s for s in self.samples if s[0] != hadm_id]
            ...
            full_baseline_tensor = self.get_baseline(hadm_id)
            # Ensure baseline tensor is a standalone tensor if it exists
            if full_baseline_tensor is not None and torch.is_tensor(full_baseline_tensor):
                full_baseline_tensor = full_baseline_tensor.clone().detach().contiguous()

            # Optionally save the chunk slice.
    ...
    def get_baseline(self, hadm_id):
        ...
        """
        meta = self.select_if_exists(self.splits_loader.merged_with_disch_df, hadm_id)
        if meta.empty:
            ...
            row = meta.iloc[0]
            try:
                age_value = float(row['age']/100) #to normalize
            except Exception:
                age_value = 0.0
